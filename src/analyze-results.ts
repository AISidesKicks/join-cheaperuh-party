...
interface SupervisorDecision { taskId: string; category: string; choice: number; effort: ReasoningEffort; usage?: Usage }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const tasks = DEMO_TASKS.map((task) => {
    ...
    const supervisor = decisions.filter((decision) => decision.taskId === task.id);
    ...
    return { taskId: task.id, category: task.category, ...worker, supervisor: { decisions: supervisor.length, tokens: supervisorTokens, cost: supervisorCost }, combinedCost: worker.totalCost + supervisorCost };
  });
  const report = { ignoredDuplicateRecords: records.length - canonical.length, workerTotals: { attempts: canonical.length, tokens: canonical.map(tokens).reduce((sum, value) => sum + value, 0), cost: canonical.map(cost).reduce((sum, value) => sum + value, 0) }, supervisorTotals: { decisions: decisions.length, tokens: decisions.map(tokens).reduce((sum, value) => sum + value, 0), cost: decisions.map(cost).reduce((sum, value) => sum + value, 0) }, tasks };
  if (process.argv.includes("--write")) await writeFile("results/openrouter-supervisor-demo-summary.json", `${JSON.stringify(report, null, 2)}\n`);
