export const REASONING_LEVELS = [0, 12, 25, 38, 50, 63, 75, 88, 100] as const;
...
interface CategoryTemplate { id: string; cases: SeedCase[] }

const categories: CategoryTemplate[] = [
  { id: "one-line-repair", cases: [
  ...
  { id: "debug-diagnosis", cases: [
    { prompt: "A request sometimes serves English to Czech users because the cache key omits locale. Return only the root cause.", expected: "locale missing from cache key", matchMode: "contains" }, { prompt: "A form submits twice because both click and submit handlers call save. Return only the root cause.", expected: "duplicate save handlers", matchMode: "contains" }, { prompt: "A timer leaks after navigation because cleanup is never called. Return only the root cause.", expected: "missing timer cleanup", matchMode: "contains" },
  ...
  { id: "test-design", cases: [
    { prompt: "A function accepts an inclusive range. Name one essential edge-case test. Return only the test input.", expected: "start equals end", matchMode: "contains" }, { prompt: "A password validator has minimum length 8. Name one boundary test. Return only the length.", expected: "7" }, { prompt: "A pagination API accepts page >= 1. Name the invalid boundary. Return only the page number.", expected: "0" },
...
export const TASKS: BenchmarkTask[] = categories.flatMap((category) => category.cases.map((testCase, index) => ({
...
export function reasoningRequest(effort: ReasoningEffort): Record<string, unknown> {
  return effort === 0 ? { enabled: false } : { effort };
}

export function directDeepSeekThinkingRequest(effort: ReasoningEffort): Record<string, unknown> {
  return effort === 0 ? { thinking: { type: "disabled" } } : { thinking: { type: "enabled" }, reasoning_effort: effort };
}
