...
const categories: CategoryTemplate[] = [
  ...
  { id: "debug-diagnosis", brief: "A user reports an application symptom and needs its likely root cause.", cases: [
    { prompt: "A request sometimes serves English to Czech users because the cache key omits locale. Return only the root cause.", expected: "omits locale", matchMode: "contains", brief: "A user reports a cross-locale cache symptom with one stated cause." }, { prompt: "A form submits twice because both click and submit handlers call save. Return only the root cause.", expected: "duplicate save handlers", matchMode: "contains", brief: "A user reports duplicate persistence from overlapping UI events." }, { prompt: "After sorting a translated product list, clicking the second row opens details for a different product. The rows use their current array index as the key. Return only the root cause.", expected: "unstable index key", matchMode: "contains", brief: "A user reports a UI identity bug that appears only after reordering data." },
  ...
  { id: "test-design", brief: "A user needs to identify a valuable boundary or invariant test for a behavior.", cases: [
    { prompt: "A function accepts an inclusive range. Name one essential edge-case test. Return only the test input.", expected: "1,1", brief: "A user needs one boundary test for an inclusive range contract." }, { prompt: "A password validator has minimum length 8. Name one boundary test. Return only the length.", expected: "7", brief: "A user needs the immediate invalid boundary for a stated threshold." }, { prompt: "A pagination API accepts page >= 1 and rejects non-integers. Name the closest invalid numeric boundary. Return only the page value.", expected: "0", brief: "A user needs a boundary test from a small input-domain contract." },
